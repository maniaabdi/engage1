...
/*engage1*/
DataCache::DataCache () 
	: cache_enable(0), cache_lock("cache_mutex"), 
	io_ctx(0), request_lock("request_mutex"), 
	rgw_data_cache_request_window_size(0), 
	cct(NULL), io_type(ASYNC_IO), 
	thread_name("cache_completion"), 
	tmp_data(NULL), max_chunk_size(0x400000), finisher_thread(this){}
...
void *DataCache::finisher_thread_entry(){

	int n_recv_eve;
	cacheAioRequest *responce;
	dout(0) << "DataCache::finisher_thread_entry cache finisher thread start--- Engage1" << dendl;
	while (cache_enable) {
		n_recv_eve = io_getevents(io_ctx, 1, rgw_data_cache_request_window_size, aio_events, NULL);
		if (n_recv_eve < 1)
		{
		     cache_enable = 0;
		     continue;
		}
		for (int i = 0; i < n_recv_eve; i++) {
			responce = (cacheAioRequest *)aio_events[i].data;
			responce->op_data->cache_get_completed_ios(responce);
		}
	}
...
int DataCache::request_queue_setup(){

	int ret = 0;
	if (!cache_enable)
	{
		ret = -1;
		dout(0) << "Error: DataCache::request_queue_setup: failed. (data cache disabled) --- Engage1" << dendl;
		goto END;
	}

	tmp_data = new char [max_chunk_size];

	aio_events = new struct io_event[rgw_data_cache_request_window_size];
	memset(aio_events, 0, rgw_data_cache_request_window_size*sizeof(struct io_event));

	ret = io_setup(rgw_data_cache_request_window_size, &io_ctx);
	if (ret < 0)
	{
		cache_enable = 0;
		dout(0) << "Error: DataCache::request_queue_setup failed. io_setup return:" << ret << dendl;
		goto END;
	}
...
int DataCache::operate_io_read(bufferlist *bl, unsigned int len, std::string oid){
...
int DataCache::operate_aio_read(cacheAioRequest *c) {

	int r = 0;	

	struct iocb *aiocbp = new struct iocb;
	memset(aiocbp, 0, sizeof(iocb));
