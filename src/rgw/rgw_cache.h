...
struct DataCache {
	...
	private:
		...
		io_context_t io_ctx;   
		Mutex  request_lock;
		int rgw_data_cache_request_window_size;
		CephContext *cct;
		enum _io_type { 
			SYNC_IO = 1,
			ASYNC_IO = 2,
		...
		struct FinisherThread : public Thread {
			...
			explicit FinisherThread(DataCache *c) : data_cache(c) {}
			void* entry() { return (void*)data_cache->finisher_thread_entry(); }
		} finisher_thread;

	private:
		void add_io();
		int request_queue_setup();
	public:
		DataCache();
		~DataCache();
		...
		int operate_io_read(bufferlist *bl, unsigned int len, std::string oid);
		int operate_aio_read(struct cacheAioRequest *cc);

		void set_ctx(CephContext *_cct) {
			cct = _cct;
			rgw_data_cache_request_window_size = cct->_conf->rgw_data_cache_request_window_size; 
			cache_enable = 1;
			request_queue_setup();
		}
...
int RGWCache<T>::read_from_cache(bufferlist *bl, int len, std::string oid)
{
	...
	return data_cache.operate_io_read(bl, len , oid);
}
...
int RGWCache<T>::iterate_obj(RGWObjectCtx& obj_ctx, rgw_obj& obj,
		...
		void *arg)
{
	return T::iterate_obj(obj_ctx, obj, ofs, end, max_chunk_size, iterate_obj_cb, arg);
}
	template <class T>
int RGWCache<T>::cache_aio_read(cacheAioRequest *cc)
{
	int r = T::cache_aio_read(cc);
	r = data_cache.operate_aio_read(cc);
	return r;
}
/*engage1*/
