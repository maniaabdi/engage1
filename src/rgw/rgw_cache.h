...
//#define chdout(v) lsubdout(cct, rgw, v)



/* engage1 */
struct CacheAioCompletion
{
    CacheAioCompletion() : fd(-1), buf(NULL), len(-1), pbl(NULL), cb(NULL)   {} 
    int fd;
    unsigned char *buf;
    unsigned int  len;
    bufferlist *pbl;
...
struct ObjectDataInfo {
        uint64_t size;
        time_t access_time;
        map<std::string, ChunkDataInfo> chunk_list; // entry: oid, chunkdatainfo
        std::string obj_key;
        CephContext *cct;
        ...
        void dump(Formatter *f) const;
        void put(std::string oid);
        static void generate_test_instances(list<ObjectDataInfo *>& o);
};

//static void handle_cache_read_cb(int signal, siginfo_t *info, void*uap);
...
public:
    DataCache();
    ~DataCache() { delete tmp_data; }
    
    void put( const std::string& obj_key, const std::string& oid);
    int write(bufferlist& bl, unsigned int len, std::string oid);
    void handle_data(bufferlist& bl, unsigned int len,std::string obj_key);

    //reading from cache
    bool get(std::string oid);
    int read(bufferlist *bl, unsigned int len, std::string oid);
    int aio_read(void *priv_d, librados::AioCompletion* c, bufferlist *plb, unsigned int len, string oid);
    int create_completion(void *priv_d, librados::AioCompletion* c, bufferlist *plb, unsigned int len, string oid, CacheAioCompletion *cc);
...
public:
  ...
  /*engage1*/
  void put_cache(std::string oid, std::string obj_key);
  ...
  int cache_aio_read(void *d, librados::AioCompletion* c, bufferlist *plb, unsigned int len, string oid);
...
template <class T>
int RGWCache<T>::iterate_obj(RGWObjectCtx& obj_ctx, rgw_obj& obj,
                          off_t ofs, off_t end,
                          uint64_t max_chunk_size,
                          int (*iterate_obj_cb)(rgw_obj&, off_t, off_t, off_t, bool, RGWObjState *, void *),
                          void *arg)
...
template <class T>
int RGWCache<T>::cache_aio_read(void *priv, librados::AioCompletion* c, 
                        bufferlist *pbl, unsigned int len, string oid)
{
    int r = T::cache_aio_read(priv, c, pbl, len, oid);
    r = data_cache.aio_read(priv, c, pbl, len, oid);
    return r;
}
/*engage1*/
