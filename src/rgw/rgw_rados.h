...
WRITE_CLASS_ENCODER(RGWObjManifestRule)
	...
	class RGWObjManifest {
		...
		public:
			...
			/*engage1*/
			const std::string& get_bucket_name () const  { return head_obj.get_bucket_name(); }
			const std::string& get_object_name () const { return head_obj.get_orig_obj();    }
			/*engage1*/
...
class RGWRados
{
	...
	virtual int iterate_obj(RGWObjectCtx& ctx, rgw_obj& obj,
			off_t ofs, off_t end,
			uint64_t max_chunk_size,
	...
	/*engage1*/
	virtual void put_cache( std::string oid, std::string obj_key) {}
	virtual void cache_handle_data(bufferlist bl, int len, std::string obj_key) {}
